...
impl Sha256 {
    ...
    /// Finalize, returning the 32-byte digest. Consumes the hasher.
    ///
    ...
    fn process(&mut self, block: &[u8; 64]) {
        ...
        for (i, &wi) in w.iter().enumerate() {
            ...
            let ch = (e & f) ^ ((!e) & g);
            let t1 = h.wrapping_add(s1).wrapping_add(ch).wrapping_add(K[i]).wrapping_add(wi);
